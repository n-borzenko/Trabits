  ...
  @ViewBuilder private func cell(details: (isCompleted: Bool, value: Double, target: Double), day: Int, isCurrentMonth: Bool) -> some View {
    ...
    ZStack {
      ...
      Text("\(day)")
        ...
        .alignmentGuide(VerticalAlignment.center, computeValue: { d in d[VerticalAlignment.center] + 4 })
      Circle()
        .fill(.contrast)
        .frame(width: dotSize, height: dotSize)
        .opacity(details.isCompleted ? 1 : 0)
        .alignmentGuide(VerticalAlignment.center, computeValue: { d in
          d[VerticalAlignment.center] - (dynamicTypeSize.isAccessibilitySize ? 10 : 8)
        })
    }
...
extension StatisticsMonthHabitChartView: AXChartDescriptorRepresentable {
  func makeChartDescriptor() -> AXChartDescriptor {
    let dates = results.weekProgress.enumerated().flatMap { weekIndex, weekProgress in
      (0..<7).map { index in
        guard let date = Calendar.current.date(byAdding: .day, value: weekIndex * 7 + index, to: extendedMonth.start) else { return "" }
        let weekdayIndex = (index + Calendar.current.firstWeekday - 1) % 7
