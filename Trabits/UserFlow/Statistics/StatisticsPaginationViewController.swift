...
struct StatisticsPaginationControllerWrapper: UIViewControllerRepresentable {
  ...
  func makeUIViewController(context: Context) -> StatisticsPaginationViewController {
    let controller = StatisticsPaginationViewController(
      statisticsRouter: statisticsRouter, context: managedObjectContext
    )
    return controller
  }
...
class StatisticsPaginationViewController: UIViewController {
  ...
  init(statisticsRouter: StatisticsRouter, context: NSManagedObjectContext) {
    ...
    cancellable = statisticsRouter.$currentState.sink { [weak self] newValue in
      ...
      subtitleViewContainer.rootView.subtitle = StatisticsRouter.generateTitle(
        contentType: newValue.contentType, date: newValue.date
      )
...
extension StatisticsPaginationViewController {
  private func setupViews() {
    ...
    let subtitleView = StatisticsSubtitleView(
      ...
      subtitle: StatisticsRouter.generateTitle(
    ...
    subtitleViewContainer.view.leadingAnchor.constraint(
      equalTo: view.safeAreaLayoutGuide.leadingAnchor
    ).isActive = true
    subtitleViewContainer.view.trailingAnchor.constraint(
    ...
    let initialControllers = [generatePageView(
        contentType: statisticsRouter.currentState.contentType,
        date: statisticsRouter.currentState.date
    )]
...
extension StatisticsPaginationViewController: AccessibilityPageScrollDelegate {
  func selectNextPage(direction: UIPageViewController.NavigationDirection) -> Bool {
    ...
    currentDateUpdateHandler(
      ...
      guard let self else { return }
      statisticsRouter.currentState.date = newDate
      let title = StatisticsRouter.generateTitle(
        contentType: statisticsRouter.currentState.contentType,
        date: newDate
      )
      UIAccessibility.post(notification: .pageScrolled, argument: "\(title) is selected")
    }
  ...
  func currentDateUpdateHandler(newState: StatisticsRouterState, completion: ((Bool) -> Void)? = nil) {
    if newState.contentType != statisticsRouter.currentState.contentType {
      let initialControllers = [
        generatePageView(contentType: newState.contentType, date: newState.date)
      ]
      pageViewController.setViewControllers(
        initialControllers, direction: .forward, animated: false, completion: completion
      )
      return
    }
    ...
    pageViewController.setViewControllers(
      initialControllers, direction: direction, animated: true, completion: completion
    )
  }
}

extension StatisticsPaginationViewController {
  private func generatePageView(
    contentType: StatisticsContentType, date: Date, adjustment: Int = 0
  ) -> UIViewController {
    switch contentType {
    case .weekly:
      guard let interval = Calendar.current.weekInterval(for: date, adjustment: adjustment) else {
  ...
  private func getCurrentPageDate(adjustment: Int = 0) -> Date? {
    ...
    case .weekly:
      guard let controller = pageViewController.viewControllers?.first as?
              UIHostingController<StatisticsWeekView> else {
        return nil
      }
      let startDate = controller.rootView.weekData.week.start
      guard let date = Calendar.current.weekInterval(for: startDate, adjustment: adjustment)?.start else {
        return nil
      }
      return date
    case .monthly:
      guard let controller = pageViewController.viewControllers?.first as?
              UIHostingController<StatisticsMonthView> else {
        return nil
      }
